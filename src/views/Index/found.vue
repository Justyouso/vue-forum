...
import { articleHotList,articleHotWordCloud } from "@/api/article";
import { userHotList, userFollow } from "@/api/user";
export default {
  name: "Found",
  ...
  data() {
    return {
      articleData: [], // 文章列表
      authorData: [], // 作者列表
      authorLabel: [{ label: "热门作者", operation: "换一批" }], // 作者列表头
      rotate: false, // 刷新图标
      pageInfo: {}, // 作者列表翻页信息
      wordCloudData:[]// 词云
    };
  ...
  methods: {
    ...
    getWordCloud(){
      ...
      articleHotWordCloud(requestData)
        .then(response => {
